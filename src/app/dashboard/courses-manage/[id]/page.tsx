...
export default function CourseEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  /* CSV enrollment upload state */
  const [csvFile, setCsvFile] = useState<File | null>(null);
  const [csvUploading, setCsvUploading] = useState(false);
  const [csvResult, setCsvResult] = useState<{ enrolled: number; skipped: number; failed: number; total: number; results: { email: string; status: string; error?: string }[] } | null>(null);
  ...
  async function handleCsvEnroll() {
    ...
    const token = getToken();
    if (!token) { setError("Not authenticated"); setCsvUploading(false); return; }
    try {
      const fd = new FormData();
      fd.append("file", csvFile);
      if (enrollCohortId) fd.append("cohortId", enrollCohortId);
      const res = await fetch(`/api/admin/courses/${id}/enrollments/bulk`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: fd,
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error || "Upload failed"); setCsvUploading(false); return; }
      setCsvResult(data);
      setCsvFile(null);
      await reloadEnrollments();
    } catch { setError("Network error"); } finally { setCsvUploading(false); }
  }
          ...
          <div className="bg-white/70 backdrop-blur-sm border border-white/60 shadow-md rounded-2xl p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              ...
              <div className="flex gap-2">
                ...
                <label className="px-3 py-2 text-xs font-medium border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors flex items-center gap-1.5 cursor-pointer">
                  ...
                  Import CSV
                  <input type="file" accept=".csv" className="hidden" onChange={(e) => { setCsvFile(e.target.files?.[0] || null); setCsvResult(null); }} />
                </label>
            ...
            {csvFile && (
              <div className="border border-emerald-200 rounded-xl p-4 mb-4 bg-emerald-50/30">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => { setCsvFile(null); setCsvResult(null); }} className="text-xs text-gray-400 hover:text-gray-700">Clear</button>
                </div>
                <div className="flex items-center gap-3 mb-2">
                  {cohorts.length > 0 && (
                    <select value={enrollCohortId} onChange={(e) => setEnrollCohortId(e.target.value)} className="text-xs border border-gray-200 rounded-lg px-2 py-1.5 outline-none focus:border-emerald-400 transition bg-white">
                      <option value="">No cohort</option>
                      {cohorts.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                  )}
                  <button onClick={handleCsvEnroll} disabled={csvUploading} className="px-4 py-1.5 text-sm font-semibold text-white bg-gradient-to-r from-emerald-600 to-teal-600 rounded-lg hover:from-emerald-700 hover:to-teal-700 transition-all shadow-sm disabled:opacity-60">
                    {csvUploading ? "Uploading..." : "Upload & Enroll"}
                  </button>
                </div>
                <p className="text-xs text-gray-400">CSV must have an <strong>email</strong> column. Users must already exist in the system. <a href="/sample-enrollments.csv" download className="text-emerald-600 hover:text-emerald-800 underline">Download sample</a></p>
              </div>
            )}
            ...
            {csvResult && (
              <div className="border border-emerald-200 rounded-xl p-4 mb-4 bg-emerald-50/30">
                <p className="text-sm font-medium text-gray-900 mb-2">
                  Upload complete: {csvResult.enrolled} enrolled, {csvResult.skipped} skipped, {csvResult.failed} failed (of {csvResult.total})
                </p>
                {csvResult.results.filter((r) => r.status !== "enrolled").length > 0 && (
                  <div className="max-h-32 overflow-y-auto space-y-1">
                    {csvResult.results.filter((r) => r.status !== "enrolled").map((r, i) => (
                      <p key={i} className="text-xs text-gray-500">
                        <span className={r.status === "skipped" ? "text-amber-600" : "text-red-600"}>{r.status}</span>{" "}
                        {r.email}{r.error ? ` — ${r.error}` : ""}
                      </p>
                    ))}
                  </div>
                )}
                <button onClick={() => setCsvResult(null)} className="text-xs text-gray-400 hover:text-gray-700 mt-2">Dismiss</button>
              </div>
            )}
