...
export default function EditCoursePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [stats, setStats] = useState({ enrolledCount: 0, sectionCount: 0 });
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto">
        <span className="text-sm font-semibold text-white bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-2 rounded-xl shadow-md shadow-violet-500/25">Details</span>
        <Link href={`/dashboard/courses-manage/${id}/curriculum`} className="text-sm font-medium text-gray-600 bg-white/70 border border-white/60 px-4 py-2 rounded-xl hover:bg-white/90 transition-colors">
          Curriculum ({stats.sectionCount})
        </Link>
        <Link href={`/dashboard/courses-manage/${id}/enrollments`} className="text-sm font-medium text-gray-600 bg-white/70 border border-white/60 px-4 py-2 rounded-xl hover:bg-white/90 transition-colors">
          Enrollments ({stats.enrolledCount})
        </Link>
      </div>
